#!/usr/bin/env bash
# Suiko-v1.5-small. Single-variable ablation vs v1.4 baseline:
#   CVAE: on -> off  (remove --use-cvae)
# Reuses student-v1_4-500m.kkc shard. Tests whether the BiGRU posterior
# encoder + FiLM conditioning hurts on dev — v1-small had no CVAE.
set -eu
...
RUN_DIR="$REPO/models/checkpoints/Suiko-v1.5-small"
TEACHER="$REPO/models/checkpoints/ctc-nat-41m-maskctc-student-wp/checkpoint_step_100000.pt"

mkdir -p "$RUN_DIR"
LOG="$RUN_DIR/train.log"
: > "$LOG"
echo "[launch] $(date)  — Suiko-v1.5-small (CVAE off)" > "$LOG"
