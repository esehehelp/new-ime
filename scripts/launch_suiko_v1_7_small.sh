#!/usr/bin/env bash
# Suiko-v1.7-small. Single-variable ablation vs v1.5 baseline:
#   chunks ratio: 0.10 -> 0.20  (with fineweb2 0.20 -> 0.10 to keep sum 1.0)
# CVAE off (winner from v1.5), cleaned bunsetsu 5 paths. Tests whether more
# short-phrase exposure improves dev EM.
set -eu
...
RUN_DIR="$REPO/models/checkpoints/Suiko-v1.7-small"
TEACHER="$REPO/models/checkpoints/ctc-nat-41m-maskctc-student-wp/checkpoint_step_100000.pt"

mkdir -p "$RUN_DIR"
LOG="$RUN_DIR/train.log"
: > "$LOG"
echo "[launch] $(date)  — Suiko-v1.7-small (chunks 0.20)" > "$LOG"
