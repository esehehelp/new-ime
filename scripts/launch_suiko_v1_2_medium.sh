#!/usr/bin/env bash
# Launch Suiko-v1.2-medium. Works on:
#   - Windows/local (3060): REPO="D:/Dev/new-ime", uses `uv run`
#   - vast.ai (vastai/pytorch image, 5090): REPO inferred, uses `python`
#
# Expects:
#   datasets/mixes/student-cleaned-500m.kkc  (shard, max_context_chars=32)
#   models/checkpoints/ctc-nat-41m-maskctc-student-wp/checkpoint_step_100000.pt  (KD teacher)
set -eu

# --- repo root: prefer explicit D:/Dev/new-ime, fall back to script relative
if [ -d "D:/Dev/new-ime" ]; then
    REPO="D:/Dev/new-ime"
else
    REPO="$(cd "$(dirname "$0")/.." && pwd)"
fi
RUN_DIR="$REPO/models/checkpoints/Suiko-v1.2-medium"
TEACHER="$REPO/models/checkpoints/ctc-nat-41m-maskctc-student-wp/checkpoint_step_100000.pt"

mkdir -p "$RUN_DIR"
LOG="$RUN_DIR/train.log"
: > "$LOG"
echo "[launch] $(date)  — Suiko-v1.2-medium" > "$LOG"

cd "$REPO/legacy/python"
export PYTHONIOENCODING=utf-8 PYTHONUTF8=1 PYTHONUNBUFFERED=1

# runner: uv if present (local), plain python otherwise (vast.ai)
...
"${RUNNER[@]}" -m models.src.training.train_ctc_nat \
  --train  "$REPO/datasets/mixes/student-cleaned-500m.kkc" \
  ...
  --lr 2e-4 --warmup-steps 4000 --weight-decay 0.01 --grad-clip 1.0 \
