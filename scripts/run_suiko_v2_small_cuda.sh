...
default_device="cuda"
bin_path="$repo_root/build/release/rust-train.exe"
feature_stamp="$repo_root/build/release/.rust-train-features"
...
if [[ $needs_cuda_build -eq 1 ]]; then
  echo "[run_suiko_v2_small_cuda] building rust-train release with --features cuda" >&2
  cargo build -p rust-train --release --features cuda
