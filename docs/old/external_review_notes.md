---
status: superseded_in_parts
last_updated: 2026-04-18
supersedes_note: 末尾「確認済・変更不要」節は 2026-04-18 の `docs/phase3_plan.md` により撤回済
---
...
> **注記 (2026-04-18)**: 本文書は Phase 2 終了時点の知見記録。方針改訂 (Phase 3 に CVAE・
> 1.58-bit を吸収) に合わせ末尾の「確認済・変更不要」節は撤回済み。現行方針は
> `phase3_plan.md` を参照。候補ランキング/SQLite スキーマ節 (§7) は `vision.md` に一本化し、
> ここでは経緯資料として残す。
...
当時「確認済・変更不要」としていた項目のうち最初の 2 点は、Phase 3 計画改訂で反転した。
記録として残す。

- ~~**1.58-bit を外す判断**: 正しい。int8 で十分な可能性高い。~~
  → 撤回。`phase3_plan.md` Step D3/H で 1.58-bit を研究線として同時検証する。int8 ORT は
  fallback (Step G) として並置。
- ~~**CVAE を v2 送り**: 正しい。v1.0 の経験がないと設計できない。~~
  → 撤回。v1.0 研究プロトタイプの目的が「両立可能性検証」に変わったため、CVAE を
  Step D2 で本線投入する。posterior collapse 時は LoRA 分離に撤退。
