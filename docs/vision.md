...
- `datasets/mixes/scratch-200m.jsonl` 200M rows (既存 v1 mix: chunks + zenz_llmjp
  + wiki + aozora + fineweb2 + hplt3、コンポーネント: `datasets/corpus/legacy/`)
- `datasets/corpus/bunsetsu/` 約 10M 句 (wikinews/aozora_dialogue/tatoeba/wikibooks/
...
## ライブ変換 (incremental typing) 対応 — アイデアレベル

AzooKey/Hazkey + zenz が実現している「1 keystroke ごとに候補が更新されるライブ変換」
は **v1.0 では範囲外**。CTC-NAT の構造的制約があるため、対応するなら設計段階で
追加検討が要る。記録として残す。

### 構造的課題

- **CTC-NAT の encoder は双方向 attention** のため、入力末尾に 1 token 足すと
  既存位置の表現も変わる。AR の KV-cache 再利用 (past の KV は不変) に構造的に
  噛み合わない
- 結果として **毎 keystroke で encoder 全再計算**。AR が O(1) 追加計算なのに対し、
  CTC-NAT は O(seq_len) per keystroke
- 1 keystroke 単体の fwd は 15-30ms (CPU) と許容範囲だが、30 字タイプの累積で
  900ms 級に膨らむ可能性

### 緩和策 (未実装、アイデアレベル)

1. **chunk-cached encoding**: 入力を 5-8 字 chunk で区切り chunk 単位で
   encoder output をキャッシュ。chunk 境界で attention が切れる近似解。
   IME 入力は短いので影響小と推測
2. **live mode は refinement 省略**: Mask-CTC refinement は commit 時のみ実行、
   live は greedy + beam のみ
3. **Hybrid (軽量 AR + CTC-NAT)**: ライブは蒸留 AR (10-20M、KV-cache) で、commit
   時に CTC-NAT 90M + KenLM で精密変換。2 モデル保持コスト増
4. **Mask-CTC 多様性活用**: 低信頼位置を複数補完で動的候補提示、UX の「動いてる感」
   担保

### v1.0 方針

**Space 押下時変換 (commit-only)** に限定。v1.0 の勝ち筋は「zenz と同等以上の精度
かつ桁違いに速い commit 変換」で、ライブ変換の UX は zenz に劣っても構わない。

ライブ変換は **v1.5 or v2** の検討課題。CTC-NAT の構造的利点 (非自己回帰、並列
デコード) を最大活用するのが v1.0 の目的で、AR に近づける最適化は仮説検証を濁す。
