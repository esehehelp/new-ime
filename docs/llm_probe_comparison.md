...
`datasets/eval/probe/probe.json` (348 件、7 カテゴリ) で kana→kanji 変換性能を測定し、自前 42M モデル / 既存 IME-specialized モデル (zenz / jinen) / frontier 汎用 LLM を同 bench で比較した結果。
...
| 3 | zenz-v2.5-medium | 0.7557 | ~310M | IME AR | n/a | HF (local) |
| 4 | **zenz-v3.1-small** | **0.7213** | **~90M** | IME AR | n/a | HF (local) |
| 5 | zenz-v2.5-small | 0.7184 | ~90M | IME AR | n/a | HF (local) |
| 6 | DeepSeek V4-Pro thinking | 0.7155† | 1.6T/49B-act | LLM open | yes | DeepSeek |
| 7 | zenz-v2.5-xsmall | 0.7011 | 26M | IME AR | n/a | HF (local) |
| 8 | jinen-v1-small | 0.6983 | 110M | IME AR | n/a | HF (local) |
| 9 | Gemini 3.1 Pro | 0.693 | unk | LLM closed | yes? | Google AI Studio chat |
| 10 | GLM-5.1 thinking | 0.692 | 754B/40B-act | LLM open | yes | DeepInfra |
| 11 | **Suiko-v1-small + KenLM-MoE** | **0.678** | **42M** + LM | CTC-NAT (own) | n/a | local |
| 12 | Gemma 4 31B-it | 0.675 | 31B | LLM open | n/a | DeepInfra |
| 13 | DeepSeek V4-Flash thinking | 0.644† | 284B/13B-act | LLM open | yes | DeepSeek |
| 14 | jinen-v1-xsmall | 0.6264 | ~70M | IME AR | n/a | HF (local) |
| 15 | Hatsuyume + KenLM-MoE | 0.621 | 42M + LM | CTC-NAT (own) | n/a | local |
| 16 | Qwen3.6-35B-A3B | 0.471† | 35B/3B-act | LLM open MoE | yes | DeepInfra |
| 17 | Qwen3-235B-A22B-Instruct-2507 | 0.451† | 235B/22B-act | LLM open MoE | no | DeepInfra |
| 18 | Qwen3.6-35B-A3B | 0.365† | 35B/3B-act | LLM open MoE | no | DeepInfra |
...
- 90M zenz が **754B GLM-5.1 thinking (0.692) を 3pt 上回り、235B Qwen3-Instruct を 28pt 上回る**

**汎用 LLM は IME 特化に param で勝てない**:
- Gemma 4 31B (no thinking) **0.675** — 自前 Suiko-v1 (42M) と同水準
- Qwen3 系 non-thinking は IME 特化 42M に大きく負ける (0.35-0.45)
- thinking 入れても Qwen3.6 → 0.471 で Suiko-v1 未満
- DeepSeek V4-Flash thinking (671B) でさえ 0.644 で Suiko-v1 以下

**自前モデル**:
- Suiko-v1-small + KenLM-MoE **0.678** — 42M で 31B Gemma 4 と同水準、open IME ライン (zenz/jinen) からは ‑4pt
...
- **edge (40)** — IME 用語 / 固有名詞混じり。frontier LLM が 0.85-0.95、IME 特化は 0.70-0.78。世界知識が効くカテゴリ。**Opus 4.7 / Gemini 3.1 Pro が同率最高 (0.950)**
...
Suiko-v1-small (42M + KenLM-MoE) は:
- **総合 0.678** — 235B Qwen3-Instruct (0.451) を **+23pt** 上回り、31B Gemma 4 (0.675) と同水準、754B GLM-5.1 thinking (0.692) からは ‑1.4pt
