...
//!
//! The `KenLMMixture` variant is a direct port of
//! `models/src/eval/kenlm_mixture.py::KenLMMixtureScorer` — Σ_d w_d ·
//! logp_d(prefix) with per-(prefix, weight) cache. `CategoryEstimator`
//! computes the weights per input reading using the same char-class
//! heuristics as the Python reference.
...
/// Per-reading domain weights. Port of
/// `models/src/eval/kenlm_mixture.py::CategoryEstimator` — same profiles,
/// same char-class signals, same substring tech hints. Tweaks require
/// mirrored updates both here and in the Python reference so the harness
/// and the runtime stay comparable.
