...
python3 -m datasets.tools.probe.run --models ctc-nat-30m-student-step160000 --beam 1 \
    --out-dir "$OUT/greedy" 2>&1 | grep -E "EM1=" | head -1

for A in 0.2 0.4 0.6; do
    for B in 0.3 0.6; do
        echo "=== step160k alpha=$A beta=$B ==="
        python3 -m datasets.tools.probe.run --models ctc-nat-30m-student-step160000 --beam 5 \
