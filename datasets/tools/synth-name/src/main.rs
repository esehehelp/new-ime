//! synth-name: template-based generation of sentences containing common
...
#[derive(Parser, Debug)]
#[command(name = "synth-name", about = "Generate named-entity synth rows")]
struct Cli {
    #[arg(long)]
    output: PathBuf,
...
fn main() -> Result<()> {
    ...
    eprintln!(
        "[synth-name] lexicon: surnames={} given={} places={} orgs={}",
        surnames.len(),
        given.len(),
        places.len(),
        orgs.len(),
    ...
    let target_person_place = cli.target_size - target_person_single - target_person_full
        - target_place - target_org;
    ...
    for _ in 0..target_person_place {
        ...
        let (lc_s, lc_r) = match lc_idx {
            ...
            _ => (format!("{}出身の", place_s), format!("{}しゅっしんの", place_r)),
        };
        let surface = format!("{}{}", name_s, suf_s);
        let reading = format!("{}{}", name_r, suf_r);
        write_row(
    ...
    out.flush()?;
    eprintln!("[synth-name] wrote {} rows to {}", emitted, cli.output.display());
    Ok(())
}
...
fn extend_csv(target: &mut Vec<(String, String)>, path: Option<&std::path::Path>, label: &str) -> Result<()> {
    let Some(path) = path else { return Ok(()); };
    let reader = BufReader::new(
        File::open(path).with_context(|| format!("open {} csv {}", label, path.display()))?,
    );
    let mut before = target.len();
    ...
    eprintln!("[csv] {} +{} rows from {}", label, target.len() - before, path.display());
    let _ = before;
    Ok(())
}
...
fn builtin_surnames() -> Vec<(String, String)> {
    owned(&[
        ...
        ("阿部", "あべ"), ("石川", "いしかわ"), ("山下", "やました"), ("中島", "なかじま"),
        ("石井", "いしい"), ("小川", "おがわ"), ("前田", "まえだ"), ("岡田", "おかだ"),
        ("長谷川", "はせがわ"), ("藤田", "ふじた"), ("後藤", "ごとう"), ("近藤", "こんどう"),
        ("村上", "むらかみ"), ("遠藤", "えんどう"), ("青木", "あおき"), ("坂本", "さかもと"),
        ("斉藤", "さいとう"), ("福田", "ふくだ"), ("太田", "おおた"), ("西村", "にしむら"),
        ("藤井", "ふじい"), ("金子", "かねこ"), ("岡本", "おかもと"), ("中野", "なかの"),
        ("藤原", "ふじわら"), ("三浦", "みうら"), ("原田", "はらだ"), ("中川", "なかがわ"),
        ("松田", "まつだ"), ("竹内", "たけうち"), ("小野", "おの"), ("田村", "たむら"),
        ("中山", "なかやま"), ("藤本", "ふじもと"), ("原", "はら"), ("和田", "わだ"),
        ("上田", "うえだ"), ("武田", "たけだ"), ("村田", "むらた"), ("谷口", "たにぐち"),
        ("上野", "うえの"), ("工藤", "くどう"), ("宮崎", "みやざき"), ("酒井", "さかい"),
        ("大野", "おおの"), ("今井", "いまい"), ("高木", "たかぎ"), ("丸山", "まるやま"),
        ("増田", "ますだ"), ("杉山", "すぎやま"), ("野口", "のぐち"), ("大塚", "おおつか"),
        ("小島", "こじま"), ("千葉", "ちば"), ("久保", "くぼ"), ("平野", "ひらの"),
        ("岩崎", "いわさき"), ("新井", "あらい"), ("桜井", "さくらい"), ("大西", "おおにし"),
        ("松井", "まつい"), ("野村", "のむら"), ("菊地", "きくち"), ("松尾", "まつお"),
        ("大橋", "おおはし"), ("石田", "いしだ"), ("高田", "たかだ"), ("関", "せき"),
    ])
}
...
fn builtin_given_names() -> Vec<(String, String)> {
    owned(&[
        ...
        ("健", "けん"), ("健一", "けんいち"), ("健太", "けんた"), ("大輔", "だいすけ"),
        ("和夫", "かずお"), ("正男", "まさお"), ("明", "あきら"), ("誠", "まこと"),
        ("光男", "みつお"), ("博", "ひろし"), ("進", "すすむ"), ("昇", "のぼる"),
        ("豊", "ゆたか"), ("勉", "つとむ"), ("隆", "たかし"), ("剛", "つよし"),
        ("修", "おさむ"), ("学", "まなぶ"), ("翔太", "しょうた"), ("蓮", "れん"),
        ("陽翔", "はると"), ("悠真", "ゆうま"), ("颯太", "そうた"), ("湊", "みなと"),
        ("大和", "やまと"), ("樹", "いつき"),
        ("花子", "はなこ"), ("美香", "みか"), ("恵美", "えみ"), ("由美", "ゆみ"),
        ("洋子", "ようこ"), ("幸子", "さちこ"), ("京子", "きょうこ"), ("和子", "かずこ"),
        ("久美子", "くみこ"), ("真由美", "まゆみ"), ("智子", "ともこ"), ("直子", "なおこ"),
        ("美樹", "みき"), ("裕子", "ゆうこ"), ("陽子", "ようこ"), ("美穂", "みほ"),
        ("美紀", "みき"), ("愛", "あい"), ("葵", "あおい"), ("結衣", "ゆい"),
        ("凛", "りん"), ("陽菜", "ひな"), ("美咲", "みさき"), ("さくら", "さくら"),
        ("明日香", "あすか"), ("ゆり", "ゆり"), ("ひまり", "ひまり"), ("凜", "りん"),
    ])
}

fn builtin_places() -> Vec<(String, String)> {
    owned(&[
        ("東京", "とうきょう"), ("大阪", "おおさか"), ("京都", "きょうと"), ("名古屋", "なごや"),
        ("横浜", "よこはま"), ("神戸", "こうべ"), ("札幌", "さっぽろ"), ("福岡", "ふくおか"),
        ("仙台", "せんだい"), ("広島", "ひろしま"), ("千葉", "ちば"), ("埼玉", "さいたま"),
        ("川崎", "かわさき"), ("北海道", "ほっかいどう"), ("青森", "あおもり"), ("岩手", "いわて"),
        ("宮城", "みやぎ"), ("秋田", "あきた"), ("山形", "やまがた"), ("福島", "ふくしま"),
        ("茨城", "いばらき"), ("栃木", "とちぎ"), ("群馬", "ぐんま"), ("新潟", "にいがた"),
        ("富山", "とやま"), ("石川", "いしかわ"), ("福井", "ふくい"), ("山梨", "やまなし"),
        ("長野", "ながの"), ("岐阜", "ぎふ"), ("静岡", "しずおか"), ("愛知", "あいち"),
        ("三重", "みえ"), ("滋賀", "しが"), ("兵庫", "ひょうご"), ("奈良", "なら"),
        ("和歌山", "わかやま"), ("鳥取", "とっとり"), ("島根", "しまね"), ("岡山", "おかやま"),
        ("山口", "やまぐち"), ("徳島", "とくしま"), ("香川", "かがわ"), ("愛媛", "えひめ"),
        ("高知", "こうち"), ("佐賀", "さが"), ("長崎", "ながさき"), ("熊本", "くまもと"),
        ("大分", "おおいた"), ("宮崎", "みやざき"), ("鹿児島", "かごしま"), ("沖縄", "おきなわ"),
        ("渋谷", "しぶや"), ("新宿", "しんじゅく"), ("池袋", "いけぶくろ"), ("品川", "しながわ"),
        ("秋葉原", "あきはばら"), ("銀座", "ぎんざ"), ("上野", "うえの"), ("浅草", "あさくさ"),
        ("六本木", "ろっぽんぎ"), ("恵比寿", "えびす"), ("原宿", "はらじゅく"), ("表参道", "おもてさんどう"),
        ("梅田", "うめだ"), ("難波", "なんば"), ("天王寺", "てんのうじ"), ("心斎橋", "しんさいばし"),
        ("中野", "なかの"), ("西新宿", "にしんしんじゅく"), ("東京駅", "とうきょうえき"),
        ("羽田", "はねだ"), ("成田", "なりた"), ("関西", "かんさい"), ("関東", "かんとう"),
        ("東北", "とうほく"), ("九州", "きゅうしゅう"), ("四国", "しこく"), ("中部", "ちゅうぶ"),
        ("日本", "にほん"), ("アメリカ", "あめりか"), ("中国", "ちゅうごく"), ("韓国", "かんこく"),
        ("イギリス", "いぎりす"), ("フランス", "ふらんす"), ("ドイツ", "どいつ"), ("イタリア", "いたりあ"),
    ])
}

fn builtin_orgs() -> Vec<(String, String)> {
    owned(&[
        ("東京", "とうきょう"), ("日本", "にっぽん"), ("三菱", "みつびし"), ("三井", "みつい"),
        ("住友", "すみとも"), ("ソニー", "そにー"), ("ホンダ", "ほんだ"), ("トヨタ", "とよた"),
        ("日立", "ひたち"), ("富士通", "ふじつう"), ("キヤノン", "きやのん"), ("ニコン", "にこん"),
        ("任天堂", "にんてんどう"), ("楽天", "らくてん"), ("野村", "のむら"), ("大和", "だいわ"),
        ("東海", "とうかい"), ("西日本", "にしにほん"), ("東日本", "ひがしにほん"),
        ("早稲田", "わせだ"), ("慶應", "けいおう"), ("京都", "きょうと"), ("北海道", "ほっかいどう"),
        ("東北", "とうほく"), ("九州", "きゅうしゅう"), ("筑波", "つくば"), ("名古屋", "なごや"),
        ("朝日", "あさひ"), ("読売", "よみうり"), ("毎日", "まいにち"), ("日経", "にっけい"),
        ("産経", "さんけい"), ("NHK", "えぬえいちけー"), ("フジ", "ふじ"),
    ])
}
