...
use rust_model::{ctc_nat_preset, estimate_ctc_nat_resources, BatchShape, RuntimeAssumptions};
...
fn fit(
    ...
    train_log!("[rust-train] ensuring run dir");
    ensure_run_initialized(config_path, run_dir, resume)?;
    train_log!("[rust-train] loading config");
    let config = load_config(config_path)?;
    train_log!("[rust-train] loading tokenizer");
    let tokenizer = load_tokenizer(&config)?;
    train_log!("[rust-train] reading manifest/state");
    let manifest = read_manifest(&run_dir.join("run_manifest.json"))?;
    let state_path = run_dir.join("trainer_state.json");
    let mut state = read_state(&state_path)?;
    let mismatches = collect_resume_mismatches(&config, &manifest, &state);
    ...
    let effective_grad_clip = if grad_clip > 0.0 {
        if grad_clip != config.backend.grad_clip {
            eprintln!(
                "[rust-train] grad_clip override: config={} cli={} (using cli)",
        ...
        if let Some(entry) = last_checkpoint_entry(&state) {
            if entry.kind == backend.kind() {
                ...
                if backend_checkpoint.exists() {
                    train_log!(
                        "[rust-train] loading backend checkpoint {}",
    ...
    train_log!("[rust-train] startup summary");
    print_train_startup(
        &config,
        run_dir,
        device,
    ...
    train_log!("[rust-train] entering training loop");
    ...
    train_log!("[rust-train] training loop finished");
...
fn ensure_run_initialized(config_path: &Path, run_dir: &Path, resume: bool) -> Result<()> {
    ...
    if !resume {
        if has_manifest || has_state {
            eprintln!(
                "[rust-train] starting fresh from config; reinitializing {}",
                run_dir.display()
            );
        } else {
            eprintln!(
                "[rust-train] run dir is not initialized; creating {}",
                run_dir.display()
            );
        }
...
fn print_train_startup(
    ...
    steps: usize,
    checkpoint_every: usize,
    estimate: &rust_model::ResourceEstimate,
    plan: &rust_data::BatchPlan,
    epoch_steps: usize,
...
fn load_probe_items(path: &Path, limit: usize) -> Result<Vec<ProbeItem>> {
    ...
    train_log!(
        "[rust-train] loading probe items path={} bytes={} limit={}",
        path.display(),
        bytes,
        limit
    );
    ...
    train_log!(
        "[rust-train] probe file read ok chars={} elapsed_sec={:.3}",
        text.chars().count(),
        started.elapsed().as_secs_f64()
    );
    train_log!("[rust-train] probe parse start");
    ...
    train_log!(
        "[rust-train] probe parse ok items={} elapsed_sec={:.3}",
        items.len(),
        started.elapsed().as_secs_f64()
    );
    Ok(items)
...
fn evaluate_probe(
    ...
    train_log!("[rust-train] probe eval start items={}", items.len());
    let mut hits = 0usize;
    ...
    for (idx, item) in items.iter().enumerate() {
        ...
        *per_cat_total.entry(category).or_insert(0) += 1;
        if (idx + 1) % 50 == 0 || idx + 1 == items.len() {
            train_log!(
                "[rust-train] probe eval progress {}/{}",
                idx + 1,
                items.len()
            );
        }
    ...
    train_log!("[rust-train] probe eval done items={}", items.len());
...
fn model_estimate(config: &TrainConfig) -> Result<rust_model::ResourceEstimate> {
