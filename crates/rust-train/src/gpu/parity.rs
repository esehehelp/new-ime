...
//!
//! The fixtures land in `<repo>/parity-fixtures/`. They are not checked in.
//! Tests here `return` early when the fixture is missing so a clean clone
//! still passes CI.
//!
...
mod tests {
    ...
    fn skip_if_missing(path: &Path) -> bool {
        ...
        eprintln!(
            "skip: parity fixture {} missing\n  hint: regenerate it with the archived parity helper if needed",
            path.display()
        );
        true
    }
