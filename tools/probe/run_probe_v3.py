"""probe_v3 runner: category × backend EM/CharAcc/latency.

probe_v3 は AJIMEE 互換 JSON (+ category) で長文 + 文脈付き。probe_v2 の
tsv/phrase 短句版は `tools/probe/legacy/run_probe_v2.py` に退避済み。

Usage:
    uv run python -m tools.probe.run_probe_v3
    uv run python -m tools.probe.run_probe_v3 --models ctc_nat_30m_v2
    uv run python -m tools.probe.run_probe_v3 --lm-path models/kenlm/... --alpha 0.4 --beta 0.6 --beam 5
...
def build_models(
    ...
    pool: list[tuple[str, callable]] = [
        ...
        ("ctc_nat_30m-step16000",
            lambda: _ctc("models/checkpoints/ctc_nat_30m/checkpoint_step_16000.pt")),
        ("ctc_nat_30m-final",
            lambda: _ctc("models/checkpoints/ctc_nat_30m/final.pt")),
        ("ctc_nat_30m_v2-best",
            lambda: _ctc("models/checkpoints/ctc_nat_30m_v2_dryrun/best.pt")),
        ("ctc_nat_30m_v2-final",
            lambda: _ctc("models/checkpoints/ctc_nat_30m_v2_dryrun/final.pt")),
        ("ctc_nat_30m_v2-step160000",
            lambda: _ctc("models/checkpoints/ctc_nat_30m_v2_dryrun/checkpoint_step_160000.pt")),
        ("teacher-v1-150m-step200000",
            lambda: TeacherBackend(
                "models/checkpoints/teacher-v1-150m/checkpoint_step_200000.pt",
                device=device)),
    ]
    if filter_substr:
...
def main() -> None:
    ...
    ap.add_argument("--out-dir", default="results/probe_v3")
    ...
    print(f"probe_v3: {len(items)} items, {len(cats)} categories, {with_ctx} with context", flush=True)
