...
原文 (漢字かな混じり)
    │
    ▼ MeCab + unidic-lite (features[9] = 発音形出現形)
形態素列: [(表層, 読み, 品詞), ...]
    │
    ▼ jaconv でカタカナ→ひらがな統一
    │
...
| ソース | 生ペア数 | フィルタ後 | 保持率 | 品質 (監査) |
|--------|---------|-----------|--------|------------|
| Wikipedia JA | 26,838,817 | **18,426,765** | 68.7% | 87.8% clean |
| 青空文庫 | 3,758,957 | **2,420,414** | 64.4% | 92.0% clean |
| **合計** | **30,597,774** | **20,847,179** | | |

MeCab feature: `features[9]` (発音形出現形) を使用。`features[7]` (書字形基本形) は漢字がそのまま返るため不可。

品質監査: 1% 抽出 x 2回で分布安定を確認。主な残存問題は `incomplete_fragment` (9%, 短い見出し的テキスト、学習に悪影響なし) と `reading_mismatch` (2%, カタカナ語の発音形ズレ)。
