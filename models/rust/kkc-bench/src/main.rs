...
fn bench_all(
    ...
    let (vs, model) = load_model_from_checkpoint(&config.backend, &checkpoint_source, out_dir)?;
    let params = vs
        .trainable_variables()
        .iter()
...
fn load_model_from_checkpoint(
    ...
    {
        "pt" => {
            let remapped = export_python_checkpoint(checkpoint_source, out_dir)?;
            let missing = vs
                .load_partial(&remapped)
                .with_context(|| format!("load partial {}", remapped.display()))?;
            let unexpected: Vec<_> = missing
                .into_iter()
                .filter(|name| name != "proposal_pos_embed.weight" && name != "ctc_head_bias.bias")
                .collect();
            if !unexpected.is_empty() {
                bail!(
                    "python checkpoint compatibility load missing required Rust tensors: {}",
                    unexpected.join(", ")
                );
            }
        }
...
fn export_python_checkpoint(checkpoint_path: &Path, out_dir: &Path) -> Result<PathBuf> {
    let cache_dir = out_dir.join("_compat");
    std::fs::create_dir_all(&cache_dir)?;
    let stem = checkpoint_path
        .file_stem()
        .and_then(|s| s.to_str())
        .context("checkpoint stem invalid")?;
    let out = cache_dir.join(format!("{stem}.rust_state.pt"));
    if out.exists() {
        return Ok(out);
    }
    let python = std::env::var("PYTHON")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("D:\\Dev\\new-ime\\.venv\\Scripts\\python.exe"));
    let script =
        PathBuf::from("D:\\Dev\\new-ime\\models\\tools\\eval\\export_ctc_nat_rust_state.py");
    ...
    if !status.success() {
        bail!(
            "python checkpoint compatibility export failed for {}",
