...
impl MultiHeadAttention {
    pub fn new(p: &Path, hidden: i64, num_heads: i64) -> Result<Self> {
        if hidden % num_heads != 0 {
            bail!(
                "MHA hidden={hidden} is not divisible by num_heads={num_heads}",
            );
        }
...
impl DecoderLayer {
    ...
    pub fn forward(
        ...
        let ffn = self.ffn_out.forward(&self.ffn_in.forward(&pre).gelu("none"));
        &x + ffn
    }
}
...
mod tests {
    ...
    fn mha_masks_out_padded_keys() {
        ...
        let mut kv_b = kv_a.shallow_clone();
