...
def build_models(args) -> list[tuple[str, callable]]:
    ...
    pool = [
        ...
        (
            "ar_v3_vast-greedy",
            lambda: ARCheckpointBackend(
                "checkpoints/ar-31m-scratch/checkpoint_step_70000.pt", beam_width=1
            ),
        ),
        (
            "ar_v3_chunks-greedy",
        ...
        (
            "ctc-nat-30m-student-step49000-greedy",
            lambda: CTCNATBackend(
                "models/checkpoints/ctc-nat-30m-student/checkpoint_step_49000.pt",
                device="cuda" if __import__("torch").cuda.is_available() else "cpu",
            ),
        ),
        (
            "teacher-150m-teacher-step100000-greedy",
            lambda: TeacherBackend(
                "models/checkpoints/teacher-150m-teacher/checkpoint_step_100000.pt",
                device="cuda" if __import__("torch").cuda.is_available() else "cpu",
            ),
        ),
    ]
    if not args.skip_beam:
        pool += [
            ...
            (
                "ar_v3_vast-beam10",
                lambda: ARCheckpointBackend(
                    "checkpoints/ar-31m-scratch/checkpoint_step_70000.pt", beam_width=10
                ),
            ),
            (
                "ar_v3_chunks-beam10",
